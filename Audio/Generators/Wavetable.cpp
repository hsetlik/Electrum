...
void Wavetable::createTables(Wave &real, Wave &imag)
{
  ...
  while ((fabs(real[(size_t)maxHarmonic]) + fabs(imag[(size_t)maxHarmonic]) < minVal) &&
         maxHarmonic)
  {
    --maxHarmonic;
  }
...
float Wavetable::makeTable(Wave &waveReal, Wave &waveImag, float scale, float bottomFreq,
                           float topFreq, size_t tablesAdded)
{
