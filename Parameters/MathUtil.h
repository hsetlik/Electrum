...
//  inline float bezierValueFor(float x1, float y1, float x2, float y2, float
//  x3, float y3, float x4, float y4, float t)
//  {
...
// }
/**
 * @brief Find the minimum distance betwen a line (defined by two points) and
 * some third point
 *
...
struct FFT
{
   ...
   Princeton University 08544          */
  {
    ...
    {           // stage L
      ...
      Ur = 1.0f;
      Ui = 0.0f;
      Wr = std::cos((float)M_PI / (float)LE1);
      Wi = -std::sin((float)M_PI / (float)LE1); // Cooley, Lewis, and Welch have "+" here
      for (j = 1; j <= LE1; j++)
      {
        for (i = j; i <= N; i += LE)
        { // butterfly
