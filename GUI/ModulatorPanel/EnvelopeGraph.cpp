...
String EnvelopeGraph::getPointName(DragPoint* pt)
{
    if(pt == &attackEnd)
        return "Attack End";
    else if(pt == &holdEnd)
...
DragPointParameterAttachment* EnvelopeGraph::getAttachmentFor(DragPoint* pt)
{
    if(pt == &attackEnd)
...
void EnvelopeGraph::mouseDown(const MouseEvent &event) 
{
...
void EnvelopeGraph::mouseDrag(const MouseEvent &event)
{
...
void EnvelopeGraph::drawHandle(Graphics&g, Component* parent, DragPoint* point, bool shouldBeFilled)
{
...
void EnvelopeGraph::dragParamChanged(const String& id, float newValue)
{
    String iStr(index);
...
float EnvelopeGraph::parameterFromPoint(const String& id, float x, float y)
{
    String iStr(index);
    auto range = state->getAPVTS()->getParameterRange(id);
    if(id == IDs::attackMs.toString() + iStr)
...
DragPoint* EnvelopeGraph::getPointWithin(const MouseEvent& e, float radius)
{
    for(auto p : points)
    {
