...
class EnvelopeGraphCore : public Component, public AsyncUpdater
{
  // just some static helpers to figure things out. . .
  static float getMaxAttackLength(Rectangle<float> &bounds) { return FATTACK * bounds.getWidth(); }

  static float getMaxHoldLength(Rectangle<float> &bounds) { return FHOLD * bounds.getWidth(); }
  static float getMaxDecayLength(Rectangle<float> &bounds) { return FDECAY * bounds.getWidth(); }
...
private:
  ...
  const std::vector<DragPoint *> points = {&attackEnd,  &attackCurve, &holdEnd,     &decayEnd,
                                           &decayCurve, &sustainEnd,  &releaseCurve};
