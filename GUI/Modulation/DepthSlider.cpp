...
void DepthSliderLookAndFeel::drawRotarySlider(Graphics &g, int x, int y, int width, int height,
                                              ...
                                              float rotaryEndAngle, Slider &)
{
  ...
  Point<float> thumbPoint(
      fArea.getCentreX() + radius * std::cos(toAngle - MathConstants<float>::halfPi),
...
DepthSlider::DepthSlider(EVT *tree, const String &src, const String &dst, int idx)
    : state(tree), sourceID(src), destID(dst), destIndex(idx),
...
void DepthSlider::resized() { slider.setBounds(getLocalBounds()); }
