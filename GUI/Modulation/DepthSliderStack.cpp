...
ModSelectButton::ModSelectButton(int &sel, int idx, const String &srcName)
    : WedgeButton(srcName + "SelectButton"), selectedIdx(&sel), index(idx), sourceID(srcName)
{
}
...
//====================================================================================================
ModCloseButton::ModCloseButton(const String &destID) : WedgeButton(destID + "CloseButton") {}
...
DepthSliderStack::DepthSliderStack(EVT *tree, const String &dst)
    : state(tree), closeButton(dst), destID(dst), selectedSliderIndex(-1)
{
  addAndMakeVisible(&closeButton);
  closeButton.onClick = [this] { this->removeSelectedSource(); };
}
...
void DepthSliderStack::addModulationFor(const String &srcID)
{
  ...
  selButton->onClick = [this, srcID] { this->selectSource(srcID); };
...
void DepthSliderStack::resized()
{
  ...
  auto closeBounds =
      closeButton.getWedgePathForCenter(lBounds.getCentreX(), lBounds.getCentreY()).getBounds();
