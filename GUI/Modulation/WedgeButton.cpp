...
Path WedgeButton::getWedgePath(float x0, float y0, float startRads, float endRads, float r1, float r2, float offsetX, float offsetY)
{
    ...
    Point<float> start1 = center.getPointOnCircumference(r1, startRads);
    Point<float> end1 = center.getPointOnCircumference(r1, endRads);

    Point<float> start2 = center.getPointOnCircumference(r2, startRads);
    Point<float> end2 = center.getPointOnCircumference(r2, endRads);
    ...
    for(int i = 0; i < WEDGE_SEGMENTS; ++i)
    {
        ...
        float angle = Math::flerp(startRads, endRads, t);
...
WedgeButton::WedgeButton(const String& name, float startRads, float endRads, float rad1, float rad2) :
Button(name),
a1(startRads),
a2(endRads),
r1(rad1),
